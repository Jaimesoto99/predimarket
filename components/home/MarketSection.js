...
function MiniCard({ market, onOpen }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 8 }}>
        ...
        <span style={{ fontSize: 10, color: C.textDim }}>prob. SÍ</span>
      </div>

      {/* Prob bar */}
      <div style={{ height: 3, borderRadius: 3, background: `${C.no}18`, overflow: 'hidden', marginBottom: 8 }}>
      ...
      <div style={{ fontSize: 10, color: C.textDim, fontVariantNumeric: 'tabular-nums' }}>
        €{(market.total_volume || 0) > 1000
          ...
          : (market.total_volume || 0).toFixed(0)} vol
      </div>
    </div>
  )
}
...
export default function MarketSection({ icon, title, markets, onOpen, emptyText }) {
  ...
  return (
    <section style={{ marginBottom: 32 }}>
      ...
      <div style={{
        ...
        <span style={{
          fontSize: 13, fontWeight: 700, color: C.text, letterSpacing: '-0.01em',
        }}>
          {title}
        </span>
        <span style={{
      ...
      >
        {markets.map(m => (
          <div key={m.id} style={{ scrollSnapAlign: 'start' }}>
            <MiniCard market={m} onOpen={onOpen} />
          </div>
        ))}
      </div>
    </section>
