...
export async function createTrade(userEmail, marketId, side, amount, market) {
  console.log('[createTrade] called with:', userEmail, marketId, side, amount)
  ...
  // No AMM pool math — price discovery is 100% P2P.
  ...
  if (market?.mid_price != null) {
    ...
    // Fallback: display price from last trade (for UI continuity only)
  ...
  console.log('[createTrade] targetPrice:', targetPrice, 'market.mid_price:', market?.mid_price, 'market.prices:', market?.prices)
  try {
    // Ensure user exists in DB (idempotent — safe to call every time)
    console.log('[createTrade] step 1: getting user')
    const userResult = await supabase.rpc('get_or_create_user', { p_email: userEmail.toLowerCase().trim() })
    console.log('[createTrade] step 1 result:', JSON.stringify(userResult?.data), 'error:', userResult?.error?.message)

    console.log('[createTrade] step 2: calling place_limit_order with targetPrice:', targetPrice)
    ...
    console.log('[createTrade] step 3 result:', JSON.stringify(data), 'error:', error?.message)

    // Fallback: if price is exactly at limit (fresh market at 0.50), use execute_trade directly
    const priceError = error?.message || data?.error || ''
    if (priceError.includes('ya es mejor') || priceError.includes('Usa compra directa')) {
      console.log('[createTrade] fallback: calling execute_trade')
      const { data: data2, error: error2 } = await supabase.rpc('execute_trade', {
        p_email: userEmail.toLowerCase().trim(), p_market_id: marketId,
        p_side: side.toUpperCase(), p_amount: numAmount,
      })
      console.log('[createTrade] execute_trade result:', JSON.stringify(data2), 'error:', error2?.message)
      if (error2) return { success: false, error: error2.message }
      if (data2 && !data2.success) return { success: false, error: data2.error }
      const fallbackResult = { ...data2, matched: true }
      console.log('[createTrade] FINAL result (fallback):', JSON.stringify(fallbackResult))
    ...
    console.log('[createTrade] step 4: calling match_orders')
    supabase.rpc('match_orders', { p_market_id: marketId }).then(null, () => {})

    const finalResult = { ...data, matched: data.matched ?? false }
    console.log('[createTrade] FINAL result:', JSON.stringify(finalResult))
    return finalResult
