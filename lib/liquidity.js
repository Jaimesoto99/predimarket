...
// ─── Dynamic pool update after a trade ───────────────────────────────────
//
// Simple linear model that runs alongside the FPMM.
// Returns updated { yesPool, noPool, currentProbability }.
//
// Rule:
//   YES trade → yesPool += size,  noPool  -= size * 0.5
//   NO trade  → noPool  += size,  yesPool -= size * 0.5
//   Both pools clamped to MIN_POOL_SIDE.

export const MIN_POOL_SIDE = 50

export function updatePoolsAfterTrade(yesPool, noPool, tradeSize, side) {
  let yp  = parseFloat(yesPool)  || MIN_LIQUIDITY_POOL
  let np  = parseFloat(noPool)   || MIN_LIQUIDITY_POOL
  const s = Math.abs(parseFloat(tradeSize) || 0)
  ...
  yp = Math.max(MIN_POOL_SIDE, yp)
  np = Math.max(MIN_POOL_SIDE, np)

  const currentProbability = clampProbability(np / (yp + np))

  return {
    yesPool:            parseFloat(yp.toFixed(2)),
    noPool:             parseFloat(np.toFixed(2)),
    currentProbability,
  }
}
