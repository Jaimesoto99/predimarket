...
async function acquireLock(supabase, jobName) {
  const lockExpiry = new Date(Date.now() - LOCK_TIMEOUT_MINUTES * 60000)

  // 1. Check current row state in JS (avoids PostgREST timestamp parsing issues)
  ...
  // Truly locked: running with a fresh (non-stale) lock
  if (row?.status === 'running' && row?.locked_at && new Date(row.locked_at) > lockExpiry) {
    return false
  }

  // 2. Acquire: set running on idle/failed rows, or on stale running rows
  const allowedStatuses = ['idle', 'failed']
  if (row?.status === 'running') allowedStatuses.push('running') // stale lock
...
async function releaseLock(supabase, jobName, result, success) {
  ...
  // Increment run_count or error_count (best-effort, RPC may not exist)
  try {
    await supabase.rpc('increment_job_counter', { p_job_name: jobName, p_success: success })
  } catch (_) {}
...
export async function runAllDueJobs() {
  ...
  const ORDER = ['ingest', 'detect', 'signals', 'create_markets', 'resolve', 'probability', 'activity', 'trust', 'graph', 'amm_metrics', 'intelligence', 'spain_signals']
  const results = []
