...
import { closeExpiredMarkets, archiveOldMarkets, rejectExpiredCandidates }
...
export async function runCreateMarkets() {
  ...
  // Step 5: Lifecycle maintenance (close expired, archive old, reject expired candidates)
  const [closedResult, archiveResult, rejectResult] = await Promise.all([
    closeExpiredMarkets(),
    archiveOldMarkets(),
...
export async function runIntelligence() {
  const [rankResult, clusterResult, trendResult, calibrateResult, topicResult] = await Promise.allSettled([
    rankMarkets(),
    clusterMarkets(),
    detectTrends(),
    calibrateMarkets(),
