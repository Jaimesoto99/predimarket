...
export async function runCreateMarkets() {
  // Guard: disabled by default in production. Set ENGINE_CREATE_MARKETS=true to enable.
  if (process.env.ENGINE_CREATE_MARKETS !== 'true') {
    return { skipped: true, reason: 'market_creation_disabled', detections: 0, created: 0 }
  }
...
// Disabled by default. Set ENGINE_RESOLVE_MARKETS=true to enable.
// The canonical resolution system is pages/api/resolve-markets.js.

export async function runResolve() {
  if (process.env.ENGINE_RESOLVE_MARKETS !== 'true') {
    return { skipped: true, reason: 'resolution_disabled_use_resolve_markets_api' }
  }
...
export async function runSpainSignals() {
  // Guard: disabled by default. Follows same flag as global market creation.
  if (process.env.ENGINE_CREATE_MARKETS !== 'true') {
    return { skipped: true, reason: 'market_creation_disabled', articles_total: 0, markets_created: 0 }
  }
