 ...
 * Optionally include action buttons so admin can correct from the email.
 *
 * @param {{
 ...
 * }} params
 */
export function buildAlertEmail({ market, reason, details, confirmYesUrl, confirmNoUrl }) {
  return `<!DOCTYPE html>
<html lang="es">
  ...
  <table width="100%" cellpadding="0" cellspacing="0">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
        ...
        ${(confirmYesUrl && confirmNoUrl) ? `
        <tr>
          <td style="padding:0 32px 24px;">
            <p style="margin:0 0 14px;font-size:14px;color:#334155;font-weight:600;">Corregir manualmente el resultado:</p>
            <table cellpadding="0" cellspacing="0">
              <tr>
                <td style="padding-right:12px;">
                  <a href="${confirmYesUrl}"
                     style="display:inline-block;background:#16a34a;color:#ffffff;text-decoration:none;padding:13px 22px;border-radius:8px;font-size:14px;font-weight:700;">
                    ✅ RESOLVER COMO SÍ
                  </a>
                </td>
                <td>
                  <a href="${confirmNoUrl}"
                     style="display:inline-block;background:#dc2626;color:#ffffff;text-decoration:none;padding:13px 22px;border-radius:8px;font-size:14px;font-weight:700;">
                    ❌ RESOLVER COMO NO
                  </a>
                </td>
              </tr>
...
/**
 * Build the 30-minute closing warning email.
 *
 * @param {{
 *   market: { id: string, title: string, oracle_type: string|null, resolution_source: string|null },
 *   impliedProbability: number,
 *   marketUrl: string,
 * }} params
 */
export function buildClosingWarningEmail({ market, impliedProbability, marketUrl }) {
  const probPct  = (impliedProbability * 100).toFixed(1)
  const oracle   = market.oracle_type || market.resolution_source || 'Manual'
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Mercado cierra en 30 min — Forsii</title>
</head>
<body style="margin:0;padding:0;background:#f1f5f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f1f5f9;padding:32px 16px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="background:#b45309;padding:24px 32px;">
            <p style="margin:0;color:#fde68a;font-size:13px;letter-spacing:0.05em;text-transform:uppercase;">Forsii · Aviso de cierre</p>
            <h1 style="margin:8px 0 0;color:#ffffff;font-size:22px;font-weight:700;">⏰ Mercado cierra en 30 minutos</h1>
          </td>
        </tr>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;">
              <tr>
                <td style="padding:16px 20px;border-right:1px solid #e2e8f0;width:50%;">
                  <p style="margin:0 0 4px;font-size:12px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.05em;">Probabilidad implícita</p>
                  <p style="margin:0;font-size:28px;font-weight:800;color:#0f172a;">${probPct}%</p>
                </td>
                <td style="padding:16px 20px;width:50%;">
                  ...
                  <p style="margin:0;font-size:15px;font-weight:600;color:#1e293b;">${escHtml(oracle)}</p>
                </td>
              </tr>
            </table>
          </td>
          ...
          <td style="padding:0 32px 24px;">
            <div style="background:#fefce8;border:1px solid #fde047;border-radius:8px;padding:14px 16px;">
              <p style="margin:0;font-size:14px;color:#713f12;">
                📩 En 30 minutos recibirás el email de confirmación de resolución con el resultado del oráculo y los botones CONFIRMAR / RECHAZAR.
              </p>
            </div>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:0 32px 28px;">
            <a href="${marketUrl}"
               style="display:inline-block;background:#1e293b;color:#ffffff;text-decoration:none;padding:13px 22px;border-radius:8px;font-size:14px;font-weight:600;">
              🔗 Ver mercado en Forsii →
            </a>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#f8fafc;padding:16px 32px;border-top:1px solid #e2e8f0;">
            <p style="margin:0;font-size:12px;color:#94a3b8;">Email automático de Forsii. No responder a este mensaje.</p>
          </td>
        </tr>
